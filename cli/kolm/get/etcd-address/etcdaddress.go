...
func Command(getKolm common.GetKolm) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "etcd-address",
		Short: "Get the etcd address of an api.",
...
func Run(ctx context.Context, k kolm.Kolm, opts Options) error {
	api, err := k.APIs().Get(ctx, opts.Name)
	if err != nil {
		return fmt.Errorf("error getting %s: %w", opts.Name, err)
	}

	fmt.Printf("http://%s:%d", api.ETCD.Host, api.ETCD.Port)
	return nil
}
